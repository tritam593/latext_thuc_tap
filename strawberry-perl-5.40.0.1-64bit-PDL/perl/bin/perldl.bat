...
sub mypdlconfig {
  ...
  eval 'require PDL' if not defined $PDL::VERSION;
...
EOF
                if (!$@) {
                   print ", OpenGL v$OpenGL::VERSION" if defined $OpenGL::VERSION;
                   print ", OpenGL::GLUT v$OpenGL::GLUT::VERSION" if defined $OpenGL::GLUT::VERSION;
                } else {
                   print ", sorry can't load module OpenGL";
                }
                print "\n";
                OpenGL::GLUT::glutInit() unless OpenGL::GLUT::done_glutInit();
		next;
	} elsif(/^-f(.*)/) {
		my $file = $1;
		if(0 == length $1) {
...
if(!$read_from_file  and  -t STDIN) {
  ...
  if ( $readlines = !$@ ) {
    ...
    readhist();
    $PERLDL::TERM->GetHistory if $PERLDL::TERM->can('GetHistory'); # if don't do this before loading OpenGL, it can load libedit via LLVM which has C symbol clashes with GNU readline
  }
}
...
if ( $readlines ){
    ...
    if (defined &OpenGL::GLUT::done_glutInit ) {
...
sub savehist {
    ...
    return if !$PERLDL::TERM->can('GetHistory');
    my @a= grep $_ && /\S/, $PERLDL::TERM->GetHistory;
    pop @a if $a[-1] =~ /^(q$|x$|\s*exit\b|\s*quit\b)/; # chop off the exit command
    return if !@a; # nothing to do (and don't overwrite)
    @a = map {s/^\s*(.*?)\s*$/$1/;$_} @a; # strip whitespace
...
sub count_tags {
    ...
    elsif($x =~ m/^Unmatched emb\w+ quote \((.)\), de\w+ at offset (\d+)/) {
	# Embedded quote: try to close it and reparse.
	$x = $1;
	return count_tags($s.$1) . $x;
    }
    elsif($x =~ m/^Mismatched closing bracket/) {
	# This is an error condition - return false and let perl parse it
	return undef;
    }
    elsif($x =~ m/^Unmatched opening bracket\(s\)\: \{\.\.(.\.\.)+/) {
	$x = $1;
	$x=~ s/\.\.//g;
	return count_tags($s.$closers{$x}) . $x; 
    }
    elsif($x =~ m/^Unmatched opening bracket\(s\)\: \{\.\.\,/) {
    ...
    print STDERR "Unknown error message '$x' from parser...\n"
	if($PERLDL::debug);
    return undef;
}
...
sub process_input { 
 ...
 multiline: { 
  ...
  if(!defined $lines || lc $lines eq 'q' || lc $lines eq 'x' || lc $lines eq 'quit') {savehist(); exit;};
...
;
  ...
  if( (!$@) and (ref $__coderef eq 'CODE')) { 
      eval { my $sig_action = mksighandle(); &$__coderef(); die $@ if($@); };
  }
...
keys similar to C<emacs>. The last 500 commands are always stored in
...
C<gv> uses L<PDL::Core/pdumpgraph> etc to visualise the given ndarray,
with all connected transformations, etc, into the given file. Requires
L<Graph> and L<GraphViz2>.
...
NOTE: This feature is used by default by L<PDL::NiceSlice>.
...
=head2 C<perldl> and L<PDL::NiceSlice>

L<PDL::NiceSlice> introduces a more convenient
slicing syntax for ndarrays. In current versions of C<perldl> and
...
L<PDL::NiceSlice> installs a filter in the
...
You can switch the L<PDL::NiceSlice> filter on and off
by typing
